...
# q2.nlargest(2).index  -> 상관관계가 강한 2개의 변수
...
from statsmodels.api import OLS, add_constant # 상수항 추가되도록 설계

var_list=['TV', 'Radio', 'Social_Media']

#(1) LinearRegression
# 반드시 결측치를 제거하고 사용해야 된다.
# x(독립변수)는 2차원 구조로 입력을 넣어야 된다. 지금은 변수가 2개 이상이라서 이미 2차원이다.
q3=data1.dropna()
lm1=LinearRegression(fit_intercept=True) #절편 안 보이고 싶으면 Flase하면 됨
...
q3_out.sort_values(ascending=False).values #내림차순으로 정렬

# (2)ols
# lm2=ols(식, 데이터셋).fit()
# lm2=ols(식, 데이터셋)
# lm3=lm2.fit()  분리할 때는 lm3처럼 따로 지정해줘야된다.
# 식: '종속~x1+C(x2)+x3-1

lm2=ols('sales~TV+Radio+Social_Media', q3).fit()

form1='Sales~' + '+'.join(var_list)  #'sales~TV+Radio+Social_Media'와 같은것
...
#(4)독립성 검정 결과
#Drug 타입과 연관성이 있는 변수는 몇개인가?연관성 있는 변수
# 가운데 가장 큰 p-value를 찾아 수소점 여섯 번째 자리 이하는 버리고 소수점 다섯번째 자리까지 기술
...
#     print((q1['forehead_ratio'] < LB) or (q1['forehead_ratio'] >  UB)) 
...
# 2표본 평균 차이 결정: ttest
# ttest_ind: 독립인 이표본 t검정, 등분산 검정 수행 후 등분산 여부를 판정
# ttest_rel: 대응인 이표본 t검정, 데이터 수와 순서 고려
...
# 4. 신뢰수준 99%에서 양측 검정을 수행하고 결과는 귀무가설 기각의 경우 Y로, 그렇지
# 않을 경우 N으로 답하시오. (답안 예시) 1.234, Y
...
# 1. 데이터를 7대 3으로 나누어 각각 Train과 Test set로 사용한다. 이 때 seed는 123으로
# 한다.
from sklearn.model_selection import train_test_split
...
# 2. 원 데이터에 있는 7개의 변수만 Feature로 사용하고 gender를 label로 사용한다.
# (forehead_ratio는 사용하지 않음)
var_list=data3.columns.drop('gender')
len(var_list)


# 3.  Test dataset를 사용하여 예측 함수
...
precision_score(test['gender'], pred_class, pos_label='Male', 2)
...
# 2. 데이터셋 재구성: 연도, 육류 종류 고려
# 대응인 t 검정
# 연도별 소비량 차이가  없는 곳으로 판단할 수 있는 육류 종류를 모두
# 적으시오 -> pivot table
...
# 3. 연도별 소비량 차이가 없는 것으로 판단할 수 있는 육류 종류를 모두
# 적으시오.
...
# 2. 육류 종류별로 회귀분석: 결정계수, mape 계산 및 누적
...
q3_out.loc[q3_out.r2.idmax(), 'mape']
...
q3['TIME'].values.reshape(-1,1)
q3['TIME'].shape
# 3. 결정계수가 가장 높은 모델의 mape
...
#['?', 'NA', '', ' '] 이런 문자들을 결측치로 읽어옴. 최신버전 판다스의 경우 이런것들을 자동적으로 결측치로 본다
data5.columns
...
# 1. 결측치가 포함된 헹은 제거한 후 진행
q3=data5.dropna()


# 2.Segmentation 값이 A 또는 D인 데이터만 사용
q3_2=q3[q3.Segmentation.isin(['A', 'D'])]
