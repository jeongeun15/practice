...
# - 대응인 t 검정
# - 연도별 소비량 차이가 없는 것으로 판단할 수 있는 육류 종류를 모두
#   적으시오. (알파벳 순서) => pvalue 누적
...
# 3. 연도별 소비량 차이가 없는 것으로 판단할 수 있는 육류 종류 [pvalue >= 0.05]
...
#  p-value를 반올림하여 소수점 넷째 자리까지 쓰고, 귀무가설을 기각하면 Y로
