...
list1.columns #['Member_number', 'Date', 'itemDescription']
...
# 1. 가장 많은 제품이 팔린 날짜에 -> 그 날짜를 기주으로 빈도가 많은 날짜 찾기
sel_date=pos1.Date.value_counts().idxmax()


# 선택된 날짜를 기준으로 가장 많이 팔린 제품의 판매 개수
...
pos2['day_name']=pd.to_datetime(pos2.Date).dt.day_name(locale='')
...
# 4. 일별 주류판매 수 -> 금토 유무 에 따라 데이터 그룹핑
